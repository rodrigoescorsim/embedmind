...
- **Full-text-only (BM25) latency trails [tantivy](https://github.com/quickwit-oss/tantivy)** — but both are sub-millisecond, and the FTOPT work closed most of the gap: EmbedMind's `search_text` @ 10k is now **p50 0.04 ms / p99 0.23 ms** vs. tantivy's 0.01 ms / 0.11 ms (~2× on p99, down from ~3× before FTOPT-8's frame-of-reference postings format), both at perfect recall on the lexical ground truth. EmbedMind also ingests ~4× faster here (3510 vs. 882 docs/s). Expected shape for a young purpose-built inverted index against a mature dedicated engine, and it does *not* reopen [ADR 0011](docs/adr/0011-full-text-indice-invertido-proprio.md) — full-text lives *inside* the single crash-safe `.mind` file by design (tantivy would add a second, independent commit source — exactly the "unrecoverable half-state after a crash" the single-WAL design rules out).
