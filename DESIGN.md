...
- Parâmetros default: `M=16`, `ef_construction=200`. `ef_search` **escala com o tamanho do índice** (ADR 0015, S16): o default cresce por patamares conforme o `node_count` (64 até ~25k, subindo até 256 em ≥100k), porque um `ef_search` fixo de 64 sub-recuperava a 100k (recall@10 ~0,93 média / 0,20 pior query) enquanto sobrava folga de latência sob o teto de 50 ms. `Query::ef_search(n)` explícito continua soberano — o escalonamento governa só o default. Distância: coseno (vetores normalizados na inserção → produto interno).
